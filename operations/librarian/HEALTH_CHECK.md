# HEALTH_CHECK.md

Wiki health report. Populated by `scripts/librarian/health_check.py`.

Last check: 2026-05-12 00:22

## Summary

- **Total issues:** 42
- **Critical:** 38
- **Warnings:** 4

## Critical Issues

- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → accent
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → pacing
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → emotional range
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → rhythm
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → breath
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → intensity
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → intimacy
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → public register
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → private register
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → narration register
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → manifesto register
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → forbidden voice traits
- Empty style bible field: `aesthetic_corpus/voice/VOICE_STYLE_CARD.md` → synthetic disclosure rule
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → core visual identity
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → recurring motifs
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → colour palette tendencies
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → composition rules
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → typography feel
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → cinematic references without copying artists
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → image prompt vocabulary
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → forbidden visual drift
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → post image format
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → video still format
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → album/track art format
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → Pacific Commons institutional visual mode
- Empty style bible field: `aesthetic_corpus/visual/VISUAL_STYLE_BIBLE.md` → Rupture Host mythic visual mode
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → sonic identity
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → rhythmic identity
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → harmonic tendencies
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → tempo zones
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → vocal treatment
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → bass/drum tendencies
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → texture/timbre motifs
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → lyrical themes
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → Strudel/Ableton integration notes
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → forbidden sonic drift
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → resynthesis rules
- Empty style bible field: `aesthetic_corpus/music/MUSIC_STYLE_BIBLE.md` → output types

## Warnings

- Unreviewed proposal: `wiki/identity/proposed_soul_updates.md`
- Unreviewed proposal: `wiki/identity/proposed_style_updates.md`
- Unreviewed proposal: `wiki/aesthetics/voice/proposed_voice_style_card_updates.md`
- Unreviewed proposal: `wiki/aesthetics/visual/proposed_visual_style_bible_updates.md`

## Checks Performed

- [x] Empty README files
- [x] Missing metadata stubs
- [x] Style bible empty fields
- [x] Unreviewed proposals
- [x] Inbox files older than 7 days
- [x] Media without descriptive notes
- [x] Broken wiki links
- [x] Duplicate source IDs